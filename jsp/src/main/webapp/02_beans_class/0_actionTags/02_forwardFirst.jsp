...
<body>
 ...
 확인되었습니다." /></h4>
		...
		<!-- ##################  -->
		<!--  페이지 이동  -->
		<!-- [1] 리다이렉팅(redirect): 클라이언트가 서버한테 fisrt.jsp를 요청. 응답을 하면서 second.jsp를 보여줌 -->
		<!-- request, response가 2번씩 일어남 -->
	<%-- 	<% response.sendRedirect("02_forwardSecond.jsp");%> --%>
		
		<!-- [2] 포워드(forward) -->
		<!-- first에서 했는데, second페이지가 나옴. -->
		<!-- 클라이언트가 서버한테 first.jsp를 요청. 클라이언트를 전혀모르게 sever측에서 second.jsp로 바꾸는 거임. 그리고 cliecnt로 보여주는 거임. -->
		<!-- 차이점: forwarding은 한번의 요청과 한 번의 응답으로 페이지를 바꿈. 즉, 서버측에서 바꿈. -->
